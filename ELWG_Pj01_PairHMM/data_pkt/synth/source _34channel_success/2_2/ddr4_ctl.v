...
wire                local_full_0;
wire                local_empty_0;
wire      [1:0]     local_rdat_0;
reg       [1:0]     local_wdat_0;
reg                 local_wen_0;
wire                local_ren_0;
(*noprune*) reg     local_error_0;


wire                local_full_1;
wire                local_empty_1;
wire      [24:0]    local_rdat_1;
reg       [24:0]    local_wdat_1;
reg                 local_wen_1;
wire                local_ren_1;
(*noprune*) reg     local_error_1;


wire                local_full_2;
wire                local_empty_2;
wire      [511:0]   local_rdat_2;
reg       [511:0]   local_wdat_2;
reg                 local_wen_2;
wire                local_ren_2;
(*noprune*) reg     local_error_2;
...
wire                read_one_pkt_from_ram;
wire                save_one_pkt_to_ddr;
wire                read_one_pkt_from_ddr;


reg       [15:0]    ddr4_save_cont;
reg       [15:0]    ddr4_read_cont; 
/*---------------------------------------------------------------------*\
                         main code 
\*---------------------------------------------------------------------*/
...
assign f0_pcie_ram_ren_0 = read_pcie_ram_en_0 && (ddr4_save_cont!=16'd10000) && ~f0_pcie_ram_ren_1;
...
assign f0_pcie_ram_ren_1 = read_pcie_ram_en_1 && (ddr4_save_cont!=16'd10000) && ~f0_pcie_ram_ren_0;
...
begin
  ...
  end else if((f0_pcie_ram_ren_0 || f0_pcie_ram_ren_1) && (pcie_ram_ren_cont==16'd7999)) begin    
    pcie_ram_ren_cont <=#D 16'd0;    
...
//begin
//  if(!ddr4_rst_n)begin	
//    read_one_pkt_from_ram <=#D 1'b0; 
//  end else if((f0_pcie_ram_ren_0 || f0_pcie_ram_ren_1) && (pcie_ram_ren_cont==16'd7998)) begin
//    read_one_pkt_from_ram <=#D 1'b1;   
//  end else begin 
//    read_one_pkt_from_ram <=#D 1'b0;   	
//  end 	
//end
...
assign read_one_pkt_from_ram = ((f0_pcie_ram_ren_0 || f0_pcie_ram_ren_1) && (pcie_ram_ren_cont==16'd7999));
...
begin
  if(!ddr4_rst_n)begin	
    ddr4_save_cont <=#D 16'b0; 
  end else if(read_one_pkt_from_ram && ~read_one_pkt_from_ddr) begin
    ddr4_save_cont <=#D ddr4_save_cont + 1;   
  end else if(read_one_pkt_from_ddr && ~read_one_pkt_from_ram) begin 
    ddr4_save_cont <=#D ddr4_save_cont - 1;   	
  end 	
end
...
begin
  if(!ddr4_rst_n)begin	
    ...
    local_wen_0 <=#D (ddr4_wen_temp || ddr4_ren_temp);    
    local_wen_1 <=#D (ddr4_wen_temp || ddr4_ren_temp); 
    local_wen_2 <=#D (ddr4_wen_temp || ddr4_ren_temp); 
  end 
end
...
begin
  if(!ddr4_rst_n)begin	
    local_wdat_0 <=#D 2'd0;
    local_wdat_1 <=#D 25'd0;
    local_wdat_2 <=#D 512'd0;        
  end else begin
    local_wdat_0 <=#D {ddr4_wen_temp,ddr4_ren_temp};    
    local_wdat_1 <=#D ddr4_addr_temp; 
    local_wdat_2 <=#D ddr4_wdat_temp; 
  end 
end
...
fifo_8x2_dis  u_fifo_8x2_dis (   //[1] wen, [0] ren
		.data   (local_wdat_0   ), 
		.wrreq  (local_wen_0    ), 
		.rdreq  (local_ren_0    ), 
		.clock  (ddr4_clk       ), 
...
fifo_8x25_dis  u_fifo_8x25_dis (  //[24:0] addr
		.data   (local_wdat_1   ), 
		.wrreq  (local_wen_1    ), 
		.rdreq  (local_ren_1    ), 
		.clock  (ddr4_clk       ), 
...
fifo_8x512_dis  u_fifo_8x512_dis (  //[511:0] data
		.data   (local_wdat_2   ), 
		.wrreq  (local_wen_2    ), 
		.rdreq  (local_ren_2    ), 
		.clock  (ddr4_clk       ), 
...
assign  local_ren_0 = ~local_empty_0 && ddr4_ready;

assign  local_ren_1 = ~local_empty_0 && ddr4_ready;

assign  local_ren_2 = ~local_empty_0 && ddr4_ready;



assign  ddr4_wen    = local_ren_0 && local_rdat_0[1];

assign  ddr4_ren    = local_ren_0 && local_rdat_0[0];

assign  ddr4_addr   = local_rdat_1[24:0];     

assign  ddr4_wdat   = local_rdat_2[511:0];
...
//begin
//  if(!ddr4_rst_n)begin	
//    save_one_pkt_to_ddr <=#D 1'b0;
//  end else if(ddr4_wen && ddr4_wen_cont==16'd1998) begin
//    save_one_pkt_to_ddr <=#D 1'b1;  
//  end else begin
//    save_one_pkt_to_ddr <=#D 1'b0;    	
//  end 	
//end
...
assign save_one_pkt_to_ddr = (ddr4_wen && ddr4_wen_cont==16'd1999);
...
//begin
//  if(!ddr4_rst_n)begin	
//    read_one_pkt_from_ddr <=#D 1'b0;
//  end else if(ddr4_ren_temp && ddr4_ren_cont==16'd1998) begin
//    read_one_pkt_from_ddr <=#D 1'b1;  
//  end else begin
//    read_one_pkt_from_ddr <=#D 1'b0;    	
//  end 	
//end

assign read_one_pkt_from_ddr = (ddr4_ren_temp && ddr4_ren_cont==16'd1999);  


always@(posedge ddr4_clk or negedge ddr4_rst_n)
begin
  if(!ddr4_rst_n)begin	
    ddr4_read_cont <=#D 1'b0;
  end else if(save_one_pkt_to_ddr && ~read_one_pkt_from_ddr) begin
    ddr4_read_cont <=#D ddr4_read_cont + 1;  
  end else if(read_one_pkt_from_ddr && ~save_one_pkt_to_ddr) begin
    ddr4_read_cont <=#D ddr4_read_cont - 1;    	
  end 	
end
...
assign  ddr4_ren_temp = ddr4_ready && (ddr4_read_cont!=0) && pcie_mem_free && ~ddr4_wen_temp;
