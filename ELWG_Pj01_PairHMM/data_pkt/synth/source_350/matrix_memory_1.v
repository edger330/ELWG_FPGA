	...
	if (!sys_rst_n) begin 
		...
		case (j)
			...
			5'd5: begin // y_initial
				...
				j <= j + 1'b1;
				//------------------------------------
				if ((| read_len[1:0]) == 1'b1 ) begin 
					sum_save_R <= (read_len >> 2) + 1'b1 ;
				end else begin
					sum_save_R <= read_len >> 2 ;
				end
				//------------------------------------
				if ((| hap_len[1:0]) == 1'b1 ) begin 
					sum_save_H <= (hap_len >> 2) + 1'b1 ;
				end else begin
					sum_save_H <= hap_len >> 2 ;
				end
			end
			...
			5'd6: begin // save R in RAM
 			...
 			5'd13: begin   // T=0
 				...
 				//------------------------------------
 				r0_a_address <= ADDR_M + col_cnt - 1'b1;
 				rden_a 		 <= 1'b1;
 				//------------------------------------
 				j 			 <= j + 1'b1;
 			end
 			...
 			5'd15: begin   // T=2
 				...
 				endcase // row_pos
 				//------------------------------------
 				r0_a_address <= ADDR_X + col_cnt - 1'b1;
 				rden_a 	     <= 1;
	 			//------------------------------------
 				j 			 <= j + 1'b1;
 			end
 			...
 			5'd17: begin   //T = 4
 				...
 				//------------------------------------
 				r0_a_address <= ADDR_Y + col_cnt - 1'b1;
 				rden_a 		 <= 1;
 				//------------------------------------
 			...
 			5'd18: begin   //T = 5
 				...
 				if (col_cnt <= 9'd1) begin
 					r0_a_address <= ADDR_M + hap_len - 2 + col_cnt;
 				end else begin
 					r0_a_address <= ADDR_M + col_cnt -2;
 				end 
 				data_a 		 <= m_write;
 				rden_a 		 <= 0;
 				wren_a 		 <= 1;
 			...
 			5'd19: begin   //T = 6
 				...
 				//------------------------------------
 				if (col_cnt <= 1) begin
 					r0_a_address <= ADDR_X + hap_len - 2 + col_cnt;
 				end else begin
 					r0_a_address <= ADDR_X + col_cnt -2;
 				end 
 				data_a 		 <= x_write;
 				rden_a 		 <= 0;
 				wren_a 		 <= 1;
 			...
 			5'd20: begin   // T = 7 
 				...
 				//------------------------------------
 				if (col_cnt <= 1) begin
 					r0_a_address <= ADDR_Y + hap_len - 2 + col_cnt;
 				end else begin
 					r0_a_address <= ADDR_Y + col_cnt -2;
 				end 
 				data_a 		 <= y_write;
 				rden_a 		 <= 0;
 				wren_a 		 <= 1;
 			...
 			5'd22: begin   // T = 9
 				rden_b 		 <= 1;
 				if((col_cnt == hap_len -1) && (row_cnt == read_len - 1) ) begin 
 					r0_b_address <= ADDR_QIDC;
 				end else if ((col_cnt == hap_len - 1) && (row_cnt < read_len - 1) ) begin 
 					r0_b_address <= ADDR_QIDC + row_cnt + 1'b1; 
 				end else begin 
 			...
 			5'd23: begin   // T = 10
 				...
 				if((col_cnt == hap_len -1) && (row_cnt == read_len - 1) ) begin 
 					r0_b_address <= ADDR_R;
 				end else if ((col_cnt == hap_len - 1) && (row_cnt < read_len - 1) && (row_pos == 2'd3) ) begin 
 					r0_b_address <= ADDR_R + (row_cnt >> 2) + 1;
 				end else begin 
 					r0_b_address <= ADDR_R + (row_cnt >> 2);
 				end

 				if ((row_cnt == read_len - 1) && col_cnt == 9'd2) begin 
					en_sum <= 1'b1;
				end 
 			...
 			5'd24: begin   // T = 11
 				...
 				if((col_cnt == hap_len -1) && (row_cnt == read_len - 1) ) begin 
 					r0_b_address <= ADDR_H;
 				end else if ((col_cnt == hap_len - 1) && (row_cnt < read_len - 1) && (col_pos == 2'd3) ) begin 
 					r0_b_address <= ADDR_H;
 				end else if (col_pos == 2'd3) begin 
 					r0_b_address <= ADDR_H + (col_cnt >> 2) + 1;
 				end else begin 
 					r0_b_address <= ADDR_H + (col_cnt >> 2);
 				end
